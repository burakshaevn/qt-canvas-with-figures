...
void Trapezoid::RemoveFromScene(QGraphicsScene* scene) {
    if (trapezoid_item_ && scene->items().contains(trapezoid_item_)) {
        scene->removeItem(trapezoid_item_);
        delete trapezoid_item_;
        trapezoid_item_ = nullptr;
    }
}

void Trapezoid::Rotate(const int degrees, QGraphicsScene* scene) {
    if (!trapezoid_item_) {
        return;
    }
